...
export default function Dashboard() {
  ...
  // Fetch notifikasi — di dalam useEffect agar tidak loop
  useEffect(() => {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', backgroundColor: '#F2F0FA', fontFamily: 'Nunito, sans-serif' }}>
      ...
      {/* OVERLAY notifikasi — klik di luar untuk tutup */}
      {showNotif && (
        <div
      ...
      <header style={{ position: 'sticky', top: 0, zIndex: 10, backgroundColor: 'white', padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 12, boxShadow: '0 2px 12px rgba(45,27,105,0.08)' }}>
        ...
        <div style={{ position: 'relative', zIndex: 50 }}>
          <button
            ...
            style={{ position: 'relative', width: 40, height: 40, backgroundColor: '#2D1B69', borderRadius: '50%', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2">
              <path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/>
              <path d="M13.73 21a2 2 0 0 1-3.46 0"/>
            </svg>
            {belumDibaca > 0 && (
              <span style={{ position: 'absolute', top: 4, right: 4, width: 18, height: 18, backgroundColor: '#E74C3C', color: 'white', borderRadius: '50%', fontSize: 10, fontWeight: 900, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px solid white' }}>
                {belumDibaca}
              </span>
            )}
          </button>
          ...
          {showNotif && (
            <div
              ...
              {/* Panel Header */}
              <div style={{ padding: '16px 20px', borderBottom: '1px solid #F0EEF8', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontWeight: 900, fontSize: 16, color: '#2D1B69' }}>Notifikasi</span>
                <button
                  onClick={() => {
              ...
              <div style={{ maxHeight: 380, overflowY: 'auto' }}>
                {notifikasi.length === 0 ? (
                  <div style={{ padding: 24, textAlign: 'center', color: '#AAA', fontWeight: 700 }}>
                  ...
                  <div key={notif.id}
                    ...
                    style={{ padding: '14px 20px', borderBottom: '1px solid #F8F7FC', cursor: 'pointer', backgroundColor: notif.dibaca ? 'white' : '#F5F3FF', transition: 'background 0.2s', display: 'flex', flexDirection: 'column', gap: 4 }}
                    onMouseEnter={e => e.currentTarget.style.backgroundColor = '#EEEEFF'}
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                      ...
                      {!notif.dibaca && (
                        <div style={{ width: 8, height: 8, backgroundColor: '#4A2F9E', borderRadius: '50%', flexShrink: 0, marginTop: 3 }}/>
                      )}
                    </div>
                    <div style={{ fontWeight: 600, fontSize: 12, color: '#666', lineHeight: 1.5 }}>{notif.pesan}</div>
                  </div>
                ))}
              </div>
            </div>
